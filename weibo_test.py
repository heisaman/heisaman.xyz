...
try:
    ...
    for i in range(5):
        ...
        res = c.get('favorites', count=50, page=i+1)
        ...
        for favorite in res['favorites']:
            ...
            if collection.find({"_id": favorite['status']['id']}).count() > 0:
                ...
                favorited_time = datetime.strptime(favorite['favorited_time'], "%a %b %d %H:%M:%S %z %Y")
                favorite['favorited_time_int'] = int(favorited_time.timestamp())
                favorite['favorited_time_str'] = favorited_time.strftime("%y年%m月%d日%H:%M")
